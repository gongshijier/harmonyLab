...
export class CountQuery<T> extends AbstractQuery<T> {
  ...
  // Returns the count (number of results matching the query). Uses SELECT COUNT (*) semantics.
  public async count(): Promise<number> {
    let cursor: any = typeof this.predicates == 'string' ? await this.dao.getDatabase().rawQueries(<string> this.predicates) : await this.dao.getDatabase().rawQuery(<dataRdb.RdbPredicates> this.predicates);
    try {
