...
export class BaseDao<T, K> extends AbstractDao<T, K> {
  ...
  protected bindValues(stmt: DatabaseStatement | SQLiteStatement, entity: T): void {
    stmt.clearBindings();
    let properties = GlobalContext.getContext().getValue(GlobalContext.KEY_CLS)[entity.constructor.name];
    if (properties) {
  ...
  public readEntity(cursor: any, offset: number): T {
    ...
    let properties = GlobalContext.getContext().getValue(GlobalContext.KEY_CLS)[this.entityCls.name];
    let index = 0
    // 存储Embedded修饰的Object
  ...
  public getSelectDeep(): string {
    ...
    let entity: Entity = GlobalContext.getContext().getValue(GlobalContext.KEY_CLS_RE_SHIP)[entityCls.name]
  ...
  protected loadCurrentDeep(resultSet: any, lock: boolean): T {
    ...
    let entityCls = this.getEntityCls();
    let entities: Entity = GlobalContext.getContext().getValue(GlobalContext.KEY_CLS_RE_SHIP)[entityCls.name]
